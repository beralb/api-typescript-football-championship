...
const championshipBoardQuery = `SELECT t.team_name AS 'name',
...
const getChampionshipBoard = async (): Promise<{
  championshipBoard: ILeaderboardHome[] | unknown;
}> => {
  const [championshipBoard] = await Sequelize.query(championshipBoardQuery);
  return { championshipBoard };
};

export default getChampionshipBoard;
